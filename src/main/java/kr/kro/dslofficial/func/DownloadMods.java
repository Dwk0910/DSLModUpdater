...
public class DownloadMods {
    public static Map<Integer, String> statusMap = new TreeMap<>();
    public static Map<Integer, String> prnMap = new HashMap<>();
    public static void run(JSONArray mods, File targetFolder) {
        ...
        try {
            ...
            for (int i = 0; i < mods.length(); i++) {
                ...
                statusMap.put(idx, ColorText.text("다운로드 대기중", "gray", "none", false, false, false));

                JSONObject mod = mods.getJSONObject(i);

                service.submit(() -> {
                    statusMap.put(idx, ColorText.text("다운로드 중", "yellow", "none", false, false, false));
                    if (downloadFileFromURL(mod.getString("URL"), new File(targetFolder.toPath() + File.separator + "MOD_" + idx + ".jar"))) {
                        statusMap.put(idx, ColorText.text("다운로드 성공", "b-blue", "none", true, false, false));
                    } else
                        statusMap.put(idx, ColorText.text("다운로드 실패", "red", "none", true, false, false));
                });
            }
            ...
            do {
                if (!statusMap.equals(prnMap)) {
                    ...
                    for (int key : statusMap.keySet()) {
                        ...
                        Main.out.println(" " + ColorText.text("[ " + key + " ".repeat(digit - Integer.toString(key).length()) + "]", "green", "none", true, false, false) + " 번 째 모드..... " + statusMap.get(key));
                    }
                    Main.out.flush();
                }
